          ...
          <transition name="fade-slide" mode="out-in">
            <div :key="activeTab" class="form-wrapper">
              ...
              <div v-if="activeTab === 'Schedule Biometrics'">
                ...
                <div class="filters">
                  <div class="search-group">
                    <label>Search: <input type="text" v-model="searchTermSchedule" placeholder="ID or Name..." class="search-input" /></label>
                  </div>
                  <label>
                    Department:
                  ...
                  <button @click="scheduleSelected" :disabled="isLoading" class="action-btn view-btn">
                  ...
                  <table class="data-table">
                    ...
                    <tbody>
                      <tr v-for="app in filteredEmployees" :key="app.personID">
                        ...
                        <td>{{ app.fullName || `${app.firstName} ${app.middleName} ${app.lastName || app.surname}` }}</td>
                        <td>{{ app.departmentName || 'N/A'}}</td>
                        <td>{{ app.designation || 'N/A' }}</td>
                        <td>{{ formatDate(app.dateOfBirth) }}</td>
                        <td>
                          <span v-if="app.status === 0" class="status-badge schedule">For Schedule</span>
                      ...
                      <tr v-if="filteredEmployees.length === 0">
                        <td colspan="7" style="text-align: center; padding: 20px;">
                          No pending biometrics found for scheduling.
                        </td>
                      </tr>
                    </tbody>
                  </table>
              ...
              <div v-if="activeTab === 'For Approval'">
                <h2 class="sub-title">Pending Schedule Requests</h2>

                <div class="filters">
                  <div class="search-group">
                    <label>Search: <input type="text" v-model="searchTermApproval" placeholder="ID or Name..." class="search-input" /></label>
                  </div>
                  <label>
                    Department:
                    <select v-model="selectedDepartmentApproval">
                      <option value="">All Departments</option>
                      <option v-for="dept in departmentListApproval" :key="dept" :value="dept">
                        {{ dept }}
                      </option>
                    </select>
                  </label>

                  <button @click="approveBatchSchedule"
                          :disabled="selectedRowsApproval.length === 0 || isLoading"
                          class="action-btn view-btn">
                    Confirm Selected ({{ selectedRowsApproval.length }})
                  </button>
                </div>
                ...
                <div class="table-container">
                  <table class="data-table">
                    <thead>
                      <tr>
                        <th>
                          <input type="checkbox" v-model="selectAllApproval" @change="toggleSelectAllApproval" />
                        </th>
                        <th>Employee ID</th>
                        <th>Full Name</th>
                        <th>Department</th>
                    ...
                    <tbody>
                      <tr v-for="app in filteredApproval" :key="app.personID">
                        <td>
                          <input type="checkbox" v-model="selectedRowsApproval" :value="app.personID" />
                        </td>
                        <td class="id-column">{{ app.employeeID || 'N/A' }}</td>
                        <td>{{ app.fullName || `${app.firstName} ${app.lastName || app.surname}` }}</td>
                        <td>{{ app.departmentName || 'N/A' }}</td>
                        <td>{{ app.designation }}</td>
                        <td>
                          <div class="date-approval-cell">
                            <small class="requested-label">Requested: {{ formatDate(app.requestedDate) }}</small>
                            <input type="date"
                                   v-model="app.dateScheduleLocal"
                                   class="table-date-input" />
                          </div>
                        </td>
                        <td>
                          <button @click="approveSingleSchedule(app)" class="action-btn view-btn sm">
                            Confirm
                          </button>
                        </td>
                      </tr>
                      <tr v-if="filteredApproval.length === 0 && !isLoading">
                        <td colspan="7" class="text-center p-4">No pending schedule requests found.</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
              ...
              <div v-if="activeTab === 'Scheduled'">
                <h2 class="sub-title">Scheduled Appointments</h2>

                <div class="filters">
                  <div class="search-group">
                    <label>Search: <input type="text" v-model="searchTermScheduled" placeholder="ID or Name..." class="search-input" /></label>
                  </div>
                  <label>
                    Department:
                    <select v-model="selectedDepartmentScheduled">
                      <option value="">All</option>
                      <option v-for="dept in departmentListScheduled" :key="dept" :value="dept">
                        {{ dept }}
                      </option>
                    </select>
                  </label>
                  ...
                  <button @click="rescheduleSelected" :disabled="isLoading" class="action-btn view-btn" style="background-color: #f39c12;">
                ...
                <div class="table-container">
                  <table class="data-table">
                    <thead>
                      <tr>
                        <th>
                          <input type="checkbox" v-model="selectAllScheduled" />
                        </th>
                        <th>Employee ID</th>
                        <th>Full Name</th>
                        <th>Department</th>
                    ...
                    <tbody>
                      <tr v-for="app in filteredScheduled" :key="app.employeeID">
                        <td>
                          <input type="checkbox"
                                 v-model="selectedRowsScheduled"
                                 :value="app.personID" />
                        </td>
                        <td>{{ app.employeeID }}</td>
                        <td>{{ app.fullName }}</td>
                        ...
                        <td>{{ formatDate(app.requestedDate) }}</td>
                        <td>
                          <span class="status-badge scheduled">Scheduled</span>
                        </td>
                      </tr>
                      <tr v-if="filteredScheduled.length === 0">
                        <td colspan="7" style="text-align: center; padding: 20px;">No scheduled employees found.</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
              ...
              <div v-if="activeTab === 'Captured Biometrics'">
                <h2 class="sub-title">Captured Biometrics</h2>

                <div class="filters">
                  <div class="search-group">
                    <label>Search: <input type="text" v-model="searchTermCapture" placeholder="ID or Name..." class="search-input" /></label>
                  </div>
                  <label>
                    Department:
                    <select v-model="selectedDepartmentCapture">
                      <option value="">All</option>
                      <option v-for="dept in departmentListCapture" :key="dept" :value="dept">
                        {{ dept }}
                      </option>
                    </select>
                  </label>
                ...
                <div class="table-container">
                  <table class="data-table">
                    ...
                    <tbody>
                      <tr v-for="emp in filteredCapture" :key="emp.employeeID">
                        <td>{{ emp.employeeID }}</td>
                        <td>{{ emp.fullName }}</td>
                        <td>{{ emp.departmentName }}</td>
                        <td>{{ emp.designation }}</td>
                        <td>
                          <span :class="['status-badge', emp.status === 3 ? 'hit' : 'captured']">
                            {{ emp.status === 3 ? 'Hit Detected' : 'Captured' }}
                          </span>
                        </td>
                      </tr>
                      <tr v-if="filteredCapture.length === 0">
              ...
              <div v-else-if="activeTab === 'Generate Cards'">
                ...
                <div class="filters">
                  <div class="search-group">
                    <label>Search: <input type="text" v-model="searchTermPrinting" placeholder="App Code or Name..." class="search-input" /></label>
                  </div>
                  <label>
                    Department:
                  ...
                  <table class="data-table">
                    ...
                    <tbody>
                      <tr v-for="app in filteredCards" :key="app.applicationCode">
                        ...
                        <td>{{ formatDate(app.requestedDate) }}</td>
                        <td>
                          <img v-if="app.photo" :src="`data:image/jpeg;base64,${app.photo}`" alt="Photo" class="table-photo" />
                        </td>
                      </tr>
                      <tr v-if="filteredCards.length === 0">
                        <td colspan="6" style="text-align: center; padding: 20px;">
                          No cards ready for printing.
                        </td>
                      </tr>
                    </tbody>
                  </table>
              ...
              <div v-else-if="activeTab === 'Activate Cards'">
                <h2 class="sub-title">Activate Cards (Active Cards List)</h2>

                <div class="filters">
                  <div class="search-group">
                    <label>Search: <input type="text" v-model="searchTermActivation" placeholder="ID or Name..." class="search-input" /></label>
                  </div>
                  <label>
                    Department:
                    <select v-model="selectedDepartmentActivation">
                      <option value="">All</option>
                      <option v-for="dept in departmentListActivation"
                              :key="dept"
                              :value="dept">
                        {{ dept }}
                      </option>
                  ...
                  <button @click="activateSelectedCards"
          :disabled="selectedRowsActivation.length === 0 || isLoading"
          class="action-btn view-btn">
    {{ isLoading ? "Activating..." : `Activate ${selectedRowsActivation.length} Card${selectedRowsActivation.length === 1 ? '' : 's'}` }}
  </button>
                </div>
              ...
              <input type="checkbox"
                    ...
                    <tbody>
                      <tr v-for="app in filteredCardsActivation" :key="app.employeeID">
                        <td>
                          <input type="checkbox"
                                 v-model="selectedRowsActivation"
                        ...
                        <td>{{ formatDate(app.dateOfBirth) }}</td>
                        <td>
                          <span :class="['status-badge', STATUS_MAP[app.status]?.class || 'default']">
                            {{ STATUS_MAP[app.status]?.text || 'Unknown' }}
                          </span>
                        </td>
                      </tr>

                      <tr v-if="filteredCardsActivation.length === 0">
                        <td colspan="7" style="text-align: center; padding: 20px;">
                          No cards found for activation or release.
                        </td>
                      </tr>
                    </tbody>
                  </table>
  ...
  const refreshTab = async () => {
    switch (activeTab.value) {
      ...
      case "Captured Biometrics":
        ...
        break;
      case "Adjudication":
        await getPendingAdjudication(); // Status 3 -> GET /adjudication
        break;
      case "Generate Cards":
